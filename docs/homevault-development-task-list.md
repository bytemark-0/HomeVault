...
6. **Bug fixes** — done. *(codex — nextTaskLabel now derived from actual open tasks instead of HVAC-only hardcode; snoozed tasks re-surface as overdue/due-today when snooze date passes, fixed in both SQLite and memory repos; ensureColumn migrations added for install_date, purchase_date, cost_cents)*
