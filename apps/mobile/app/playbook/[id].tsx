...
import { ContinuityPlaybookDetailScreen } from '../../src/screens/ContinuityPlaybookDetailScreen';
...
import { canStartIncidentWorkspaceFromGuide } from '../../src/utils/incidentWorkspaces';
import { navigateBackOrReplace } from '../../src/utils/navigation';

export default function ContinuityPlaybookDetailRoute() {
    ...
    return (
      <SafeAreaView style={styles.safe}>
        <MissingRecordView
          ...
          detail="This recovery playbook is no longer available. Return to Emergency to open the current guided plans."
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ContinuityPlaybookDetailScreen
        canStartIncidentWorkspace={canStartIncidentWorkspaceFromGuide(guide)}
        canRunDrill={supportsContinuityDrill(guide)}
        guide={guide}
        onBack={() => navigateBackOrReplace('/(tabs)/emergency')}
        onOpenIncidentWorkspace={() => router.push(`/incident/${guide.id}`)}
        onOpenTarget={handleOpenTarget}
        onRunDrill={() => router.push(`/drill/${guide.id}`)}
      />
    </SafeAreaView>
  );
}
