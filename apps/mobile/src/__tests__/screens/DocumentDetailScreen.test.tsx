...
describe('DocumentDetailScreen', () => {
  ...
  it('renders linked records and opens the attached file', async () => {
    ...
    expect(await screen.findByText('Stored in HomeVault')).toBeTruthy();
    expect(screen.getAllByText('Dishwasher').length).toBeGreaterThan(0);

    fireEvent.press(await screen.findByText('Share'));
    expect(onShare).toHaveBeenCalledTimes(1);

    fireEvent.press(await screen.findByText('Open file'));
    await waitFor(() =>
      expect(Linking.openURL).toHaveBeenCalledWith(
        'file:///documents/homevault-documents/manual.pdf',
      ),
    );
  });
